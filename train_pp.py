...
def main(args):
    ...
    model = StateSpaceModel(latent_dim=latent_dim, waypoints=waypoints, sub_waypoints=sub_waypoints, metaworld=metaworld).to(device)

    optimizer = torch.optim.AdamW(model.parameters(), lr=max_lr, weight_decay=1e-12)
    ...
    if use_accelerate:
        model, optimizer, train_loader, val_loader, small_val_loader, lr_scheduler = accelerator.prepare(
        model, optimizer, train_loader, val_loader, small_val_loader, lr_scheduler)
    ...
    def train_fn(model, train_loader, val_loader, 
        ...
        for i, (expert_context, agent_traj, labels) in enumerate(iterator):
            ...
            if train and (i+1)%100==0:
                small_val_loss, small_val_loss1, small_val_loss2 = train_fn(model, train_loader, small_val_loader, lr_scheduler, optimizer, momentum_scheduler, train=False, small_val=True)
                model.train()
                if use_accelerate:
                    ...
                    small_val_loss1 = small_val_loss1.mean(dim=0).cpu().numpy() / len(small_val_loader)
                    small_val_loss2 = accelerator.gather(torch.tensor(small_val_loss2, device=device))
                    small_val_loss2 = small_val_loss2.mean(dim=0).cpu().numpy() / len(small_val_loader)
