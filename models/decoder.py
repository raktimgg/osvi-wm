...
class PerceptualLoss(nn.Module):
    """
    Perceptual (a.k.a. VGG) loss computing L1 distance between feature maps of two images.

    Args:
        layer_ids: list of ints, indices of VGG feature layers to extract
        layer_weights: list of floats, weights for each layer loss
        device: torch.device (optional)
    """
    def __init__(self, layer_ids=None, layer_weights=None, device=None):
        super().__init__()
        self.device = device or torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        # Load pre-trained VGG19 features
        vgg_features = torchvision.models.vgg19(pretrained=True).features.to(self.device).eval()
        for param in vgg_features.parameters():
            param.requires_grad = False
        self.vgg = vgg_features

        # Default to relu1_2, relu2_2, relu3_2, relu4_2 layers
        self.layer_ids = layer_ids or [1, 6, 11, 20]
    ...
    def forward(self, input, target):
        """
        input, target: tensors of shape (B,3,H,W) with values in [0,1]
        ...
        for idx, layer in enumerate(self.vgg):
            ...
            if idx in self.layer_ids:
                weight = self.layer_weights[self.layer_ids.index(idx)]
