...
def get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False):
    """
    grid_size: int or tuple/list of (grid_height, grid_width)
    return:
    pos_embed: [grid_size[0]*grid_size[1], embed_dim] or [1+grid_size[0]*grid_size[1], embed_dim] (w/ or w/o cls_token)
    """
    if isinstance(grid_size, int):  # Square grid
        grid_h, grid_w = grid_size, grid_size
    elif isinstance(grid_size, (tuple, list)):  # Non-square grid
        grid_h, grid_w = grid_size
    else:
        raise ValueError("grid_size must be an int or a tuple/list of two integers")
