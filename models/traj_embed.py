...
class NonLocalLayer(nn.Module):
    def __init__(self, in_dim, out_dim, feedforward_dim=512, dropout=0, temperature=None, causal=False, n_heads=1, dino_feat=False):
        super().__init__()
        ...
        if dino_feat:
            self._K = nn.Sequential(nn.Conv3d(in_dim, feedforward_dim, 1, stride=1, bias=False), nn.MaxPool3d(kernel_size=(1, 10, 10), stride=(1, 1, 1)))
            self._V = nn.Sequential(nn.Conv3d(in_dim, feedforward_dim, 1, stride=1, bias=False), nn.MaxPool3d(kernel_size=(1, 10, 10), stride=(1, 1, 1)))
            self._Q = nn.Sequential(nn.Conv3d(in_dim, feedforward_dim, 1, stride=1, bias=False), nn.MaxPool3d(kernel_size=(1, 10, 10), stride=(1, 1, 1)))
            self._skip = False
        else:
